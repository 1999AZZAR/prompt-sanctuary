...
class ApiKeyPool:
    ...
    def _refresh_pool(self):
        ...
        try:
            ...
            with self.get_db_connection(self.user_db_path) as conn:
                ...
                self.api_keys = [
                    key for key in self.api_keys if key.is_active or key.last_used > cutoff_time
                ]

                self.last_refresh = current_time
                logger.info(
                    f"API key pool refreshed. Active keys: {len([k for k in self.api_keys if k.is_active])}"
                )
    ...
    def get_next_key(self, exclude_user: Optional[str] = None) -> Optional[ApiKeyInfo]:
        ...
        logger.info(
            f"Selected API key for user {selected_key.username} (usage #{selected_key.usage_count})"
        )
        return selected_key
    ...
    def award_compensation(self, username: str, amount: float = None):
        ...
        try:
            ...
            add_user_points_with_source(
                ...
                amount,
                "api_key_usage",
                "Compensation for API key usage by system",
            )
    ...
    def get_user_stats(self, username: str) -> Dict:
        ...
        if not key_info:
            return {"has_key": False, "usage_count": 0, "total_compensation": 0, "last_used": None}
...
def use_system_api_key(
    user_db_path: str, exclude_user: Optional[str] = None
